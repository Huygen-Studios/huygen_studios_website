...
function ServiceRevealRow({
  ...
  const rawOpacity = useTransform(
    ...
    ([op, sp]) => {
      ...
      return (op as number) * serviceRowRevealMultiplier(
        sp as number,
        index,
        total,
        { start: innerRange[0], end: innerRange[1] }
      );
    }
  );
  ...
  const yRevealOffset = useTransform(rawOpacity, [0, 1], [40, 0]);
...
export function ServicesShowcase({
  ...
  const sectionY = useTransform(opacity, [0, 1], [40, 0]);
  const glowOpacity = useTransform(opacity, [0.5, 1], [0, 1]);
