    ...
    return (
        ...
        >
            ...
            <section className="grid place-items-center w-full relative">
                <div ref={gridFullRef} className="grid--full relative w-full mt-[5vh] mb-[2vh] h-auto aspect-[2.5] max-w-none p-4 grid gap-4 grid-cols-7 grid-rows-3">
                                    ...
                                    {bentoItems.map((bentoItem, index) => {
                                        ...
                                        return (
                                            <div
                                                key={bentoItem.id}
                                                className={cn(
                                                    "group relative cursor-pointer overflow-hidden rounded-2xl h-full transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)]",
                                                    isActive
                                                        ? "bg-zinc-900/10 shadow-2xl"
                                                        : "bg-zinc-950"
                                                )}
                                                style={{ width: isActive ? "100%" : "20%" }} // Make it 100% since it's the only card
                                                onMouseEnter={() => setActiveBento(index)}
                                                onClick={() => {
                                                    if (isActive && bentoItem.link) {
                                                        window.open(bentoItem.link, '_blank');
                                                    } else {
                                            ...
                                            >
                                                ...
                                                <div className="relative z-10 w-full h-full flex flex-col p-0">
                                                    ...
                                                    <div className={cn(
                                                        ...
                                                        <div className="absolute inset-0 bg-zinc-900 overflow-hidden z-0">
                                                            {bentoItem.image && (
                                                                <>
                                                                    ...
                                                                    />
                                                                    {/* Dark overlay that appears on hover to make text legible */}
                                                        ...
                                                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 z-20 pointer-events-none scale-95 group-hover:scale-100">
                                                            <h3 className="text-3xl lg:text-5xl font-black text-white drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)] tracking-tight">
                                                                {bentoItem.title}
                                                            </h3>
                                                        </div>
                                                    </div>
                            ...
                            return (
                                <figure key={`img-${i}`} data-col={i % 7} className="grid__item m-0 relative z-10 [perspective:800px] will-change-[transform,opacity] group cursor-pointer">
                                    <div className="grid__item-img w-full h-full [backface-visibility:hidden] will-change-transform rounded-xl overflow-hidden shadow-sm border border-zinc-200 dark:border-zinc-900 bg-zinc-100 dark:bg-zinc-950 flex items-center justify-center transition-all duration-500 ease-out group-hover:scale-105 group-hover:shadow-xl group-hover:border-transparent">
                                        ...
                                        {/* Image content */}
                                        {item && <img src={item} alt="" className="absolute inset-0 w-full h-full object-cover z-0" />}
