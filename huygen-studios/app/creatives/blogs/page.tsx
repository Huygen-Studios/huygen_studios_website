...
export default function CreativesBlogsPage() {
  ...
  return (
    <main className="min-h-screen bg-black text-white w-screen overflow-x-hidden relative font-sans">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute top-[30%] left-[20%] w-[600px] h-[600px] bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.03)_0%,transparent_70%)]" />
        <div className="absolute bottom-[10%] right-[10%] w-[400px] h-[400px] bg-[radial-gradient(ellipse_at_center,rgba(139,92,246,0.04)_0%,transparent_70%)]" />
      </div>
      ...
      <section className="w-full relative z-10 px-6 pt-[25vh] pb-24 flex flex-col items-center justify-center min-h-[50vh]">
        ...
        >
          <h1 className="text-5xl md:text-[6rem] font-medium tracking-tighter text-white mb-6 leading-[0.9]">
            The Creative
            <br />
            <span className="text-white/40 italic">Chronicles</span>
          </h1>
          <p className="text-lg md:text-xl text-white/50 max-w-2xl mx-auto leading-relaxed">
            Insights on blending cutting-edge technology with high-end aesthetic engineering.
          </p>
        </motion.div>
      </section>
          ...
          {blogArticles.map((article, idx) => (
            ...
            >
              <article className="h-full">
                <Link href={`/blog/${article.slug}`} className="flex flex-col h-full relative p-8 md:p-10 rounded-[2rem] bg-white/[0.01] border border-white/[0.05] hover:bg-white/[0.03] hover:border-white/10 transition-all duration-500 overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    <footer className="mt-auto flex items-center justify-between border-t border-white/[0.05] pt-6 group-hover:border-white/10 transition-colors">
                      ...
                      <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center group-hover:border-white/30 group-hover:bg-white/5 transition-all">
                        <ArrowRight suppressHydrationWarning size={16} className="text-white/60 group-hover:text-white group-hover:-rotate-45 transition-all duration-300" />
                      </div>
                    </footer>
                  </div>
                </Link>
