...
import { ArrowRight, Check, LayoutDashboard, Zap, Mic, Globe, Search, MessageSquare, UserCheck, Settings, TrendingUp, Video } from "lucide-react";
...
function ServicesPageInner() {
  ...
  const [activeCategory, setActiveCategory] = useState(() => {
    // Will be overridden by useEffect on mount; default to "voice"
    return "voice";
  });
  const category = CATEGORIES.find((c) => c.id === activeCategory)!;

  // Sync with ?category= query param on mount and on URL change
  useEffect(() => {
    const cat = searchParams.get("category");
    const valid = CATEGORIES.find((c) => c.id === cat);
    if (valid) setActiveCategory(valid.id);
  }, [searchParams]);
    ...
    <main className="min-h-screen bg-[#050505] text-white selection:bg-violet-500/30 font-sans overflow-x-hidden">
      ...
      <section className="relative pt-36 pb-16 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto">
        ...
        <div className="flex items-end justify-between mb-8 border-b border-white/10 pb-4">
          <h3 className="text-2xl font-semibold text-white/90 tracking-tight">Explore {category.label}</h3>
        </div>
          ...
          >
            {category.services.map((service, i) => (
              <motion.div
                ...
                initial="hidden"
                animate="visible"
                className={`relative group flex flex-col rounded-3xl border bg-gradient-to-br ${category.gradientFrom} to-transparent p-7 ${category.accentBorder} backdrop-blur-md transition-all duration-500 hover:scale-[1.015] ${service.popular ? category.glow : "hover:shadow-lg"}`}
              >
                {service.popular && (
                  <div className={`absolute -top-3 left-6 px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-widest ${category.accentBg} ${category.accent} border ${category.accentBorder}`}>
                    Most popular
                  </div>
                )}

                {service.dashboard && (
                  <div className="absolute top-5 right-5 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/[0.06] border border-white/10 text-[10px] text-white/40 font-medium">
                ...
                <div className="flex-1 flex flex-col">
                  <p className={`text-[11px] uppercase tracking-[0.2em] font-medium mb-3 ${category.accent}`}>
                    {category.label}
                  </p>

                  <h2 className="text-xl font-semibold text-white/90 tracking-tight mb-1 leading-snug">
                    {service.name}
                  </h2>
                  ...
                  <ul className="space-y-2.5 mb-8 flex-1">
                    {service.features.map((feat, fi) => (
                      <li key={fi} className="flex items-start gap-2.5">
                        <div className={`mt-0.5 flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center ${category.accentBg} border ${category.accentBorder}`}>
                          <Check size={9} className={category.accent} />
                        </div>
                ...
                <div className="flex items-center justify-between pt-5 border-t border-white/[0.08]">
                  <div>
                    ...
                    <p className={`text-2xl font-bold tracking-tight ${service.price === "Custom" ? "text-white/50 text-lg" : "text-white"}`}>
                      {service.price}
                    </p>
                  </div>
                  <Link
                    href={`/services/${service.id}`}
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 group-hover:gap-3 ${
                      service.popular
                        ? `${category.accentBg} ${category.accent} border ${category.accentBorder} hover:brightness-125`
