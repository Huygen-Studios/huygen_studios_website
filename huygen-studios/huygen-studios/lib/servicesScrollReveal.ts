...
export function servicesStageState(
  ...
  if (progress <= SERVICES_INNER.start) return { activeIndex: 0, activeProgress: 0 };
  if (progress >= SERVICES_INNER.end) return { activeIndex: total - 1, activeProgress: 1 };
  const slice = INNER / total;
  const phase = (progress - SERVICES_INNER.start) / slice;
...
export function serviceRowRevealMultiplier(
  ...
  if (progress >= SERVICES_INNER.end) return 1;
  const { activeIndex, activeProgress } = servicesStageState(progress, total);
