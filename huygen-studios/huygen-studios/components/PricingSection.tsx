...
export function PricingSection({ opacity, y, sp }: { opacity: MotionValue<number>, y: MotionValue<number>, sp: MotionValue<number> }) {
  ...
  // Card stagger animations tied to scroll percentage (sp)
  const card1OpRange = [0.805, 0.83, 0.925, 0.935];
  const card2OpRange = [0.82, 0.845, 0.925, 0.935];
  const card3OpRange = [0.835, 0.86, 0.925, 0.935];
  ...
  return (
    <motion.div 
      style={{ opacity, y, visibility: isVisible ? "visible" : "hidden" }}
      className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none"
    >
      <div className="w-full max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-16 items-center pointer-events-none">
        
        {/* Left Side: Heading Text at Top */}
        <div className="md:col-span-5 flex flex-col justify-start h-full md:pb-48">
        ...
        <div className="md:col-span-7 relative h-auto md:h-auto flex flex-row overflow-x-auto md:overflow-visible md:grid md:grid-cols-1 lg:grid-cols-3 gap-4 w-full pointer-events-auto snap-x snap-mandatory md:snap-none scroll-smooth pb-4">
          {plans.map((plan, idx) => (
            <motion.div
              key={plan.name}
              style={getCardMotionStyle(idx)}
              className={`relative rounded-2xl p-6 border pointer-events-auto flex flex-col min-w-[85vw] md:min-w-0 snap-center ${
                plan.popular
