...
export function ProcessShowcase({ 
  ...
  // Sequential highlighting logic
  // Progress range for process nodes: 0.62 to 0.77+
  const lineProgress = useTransform(scrollYProgress, [0.62, 0.76], [0, 1]);

  const innerScrollY = useTransform(scrollYProgress, (pos) => {
    const p = Math.max(0, Math.min(1, (pos - 0.62) / (0.76 - 0.62)));
    const maxScroll = -450;
    return p * maxScroll;
  });
          ...
          <div className="max-w-xl">
            ...
            <div className="space-y-8 relative">
              ...
              {PROCESS_STEPS.map((step, i) => {
                // Calculate individual step brightness based on overall progress
                // Each step gets a 25% chunk of the 0.62-0.78 range
                const stepStart = 0.62 + (i * 0.035);
                const fadeInStart = Math.max(0.61, stepStart - 0.02);
                
                // Cumulative opacity: starts dark (0.15) and stays bright (1.0)
                const stepOpacity = useTransform(
                  scrollYProgress,
                  [0.61, fadeInStart, stepStart, 1.0],
                  [0.15, 0.15, 1, 1]
                );

                const stepScale = useTransform(
                  scrollYProgress,
                  [fadeInStart, stepStart, stepStart + 0.05],
                  [0.95, 1.08, 1]
                );
